...
void udev_watch_restore(struct udev *udev)
{
        ...
        if (rename(UDEV_RUN_DIR "/watch", UDEV_RUN_DIR "/watch.old") == 0) {
                DIR *dir;
                struct dirent *ent;

                dir = opendir(UDEV_RUN_DIR "/watch.old");
                if (dir == NULL) {
                        log_error("unable to open old watches dir " UDEV_RUN_DIR "/watch.old; old watches will not be restored: %m");
                        return;
                }
...
unlink:
                ...
                closedir(dir);
                rmdir(UDEV_RUN_DIR "/watch.old");

        } else if (errno != ENOENT) {
                log_error("unable to move watches dir " UDEV_RUN_DIR "/watch; old watches will not be restored: %m");
        }
}
...
void udev_watch_begin(struct udev *udev, struct udev_device *dev)
{
        ...
        snprintf(filename, sizeof(filename), UDEV_RUN_DIR "/watch/%d", wd);
